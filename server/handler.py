...
"""
    ...
    the main thread (handler.py and server.py):
...
server.add_route('POST', lambda: True, configendpoint)
server.add_route('GET', lambda url: url.startswith('/color/set/'), writecolor)
server.add_route('GET', lambda url: url.startswith('/color/get'), readcolor)
server.add_route('GET', lambda url: url[url.rfind('.'):] in allowed_types, static)
